...
const usePermissionStore = defineStore(
    'permission',
    {
      state: () => ({
        /**
         * routes 保存了本地的公共路由和服务端返回的动态路由(菜单), 将来主要是在两个地方使用：
         * 1. 首页搜索时使用
         * 2. tagView 中使用
         *
         * 从 routes 的使用场景中可以看到, routes 变量实际上并不涉及到页面渲染, 单纯的只是处理页面的逻辑
         *
         * 这个 routes 有一个区别于下面三个变量的特点: 这个 routes 中的 children 铺平了, 即把所有的 3-n 级菜单, 全部变为 2 级菜单.
         * 这个铺平操作一共做了两件事：
         * 1. 所有的 3-n 级菜单, 全部变为 2 级菜单
         * 2. 在变的过程中, 将组件的 path 改为 父组件的path + 子组件的path
         *
